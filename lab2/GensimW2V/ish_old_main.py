...
for (
    ...
    file
):  # read the file and create list which contains all sentences found in the text
...
)  # create model using Word2Ve with the given parameters
#
    ...
    try:
        ...
        if words[0] in model:  # check if there embedding for the query word
            ...
            while k < 5:
                ...
                k += 1
            right_answers += hf.get_answer_mod(
                vectors
            )  # find the closest vector and check if it is the correct answer
